...
Compact acquires the same generation slot and UI projection as any other generation. Empty input exposes Stop; non-empty input exposes Send and enters the same FIFO pending-guidance queue. Completion, Stop, or cancellation settles the Compact Run before that queue drains through a fresh ordinary send. Automatic pre-send Compact creates the Compact boundary first and queues the triggering user input exactly once. The Compact-specific reducer subtype/effect records identity and continuation intent; it must not introduce a second composer state machine.
