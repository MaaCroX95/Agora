...
Retained Remote history has a16MiB estimated text/object budget and8192-record bound. Crossing it keeps the already loaded history and reports that remaining history must be read in Codex; it never grows Android heap or silently drops records. Runtime native-user acknowledgement survives bounded tail paging, so an off-page existing user does not disable streaming while unacknowledged input cannot start a dot. Unknown settings remain unknown, not an invented Off value. Unknown completed thought duration uses exactly Thought for a while as the English fallback, retaining genuine elapsed timings when supplied.
